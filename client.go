...
const (
	...
	userAgent      = "brand-alert-go/" + libraryVersion
	mediaType      = "application/json"
)

// defaultBrandAlertURL is the default Brand Alert API URL.
const defaultBrandAlertURL = `https://brand-alert.whoisxmlapi.com/api/v2`
...
type ClientParams struct {
	...
	// BrandAlertBaseURL is the endpoint for 'Brand Alert API' service
	BrandAlertBaseURL *url.URL
}
...
func NewClient(apiKey string, params ClientParams) *Client {
	...
	apiBaseURL := params.BrandAlertBaseURL
	if apiBaseURL == nil {
		apiBaseURL, err = url.Parse(defaultBrandAlertURL)
		if err != nil {
			panic(err)
		}
	}
	...
	client.BrandAlert = &brandAlertServiceOp{client: client, baseURL: apiBaseURL}

	return client
}

// Client is the client for Brand Alert API services.
type Client struct {
	client *http.Client
